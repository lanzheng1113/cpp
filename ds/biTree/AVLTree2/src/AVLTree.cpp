...
bool AVLTree<T,V>::insert(AVLNode<T,V>* &p,const T & e,const V& v,bool &adjusted){
       if(p == 0) {
    	   ...
    	   if(!adjusted && getHeight(p->left)-getHeight(p->right) == 2) {
    		    if(e < p->left->key)   {  //LL
                          rightRotate(p);
    		    }else {   //LR
                         leftRotate(p->left);
    	   ...
    	   if(!ret)  return ret;
    	   p->height = std::max( getHeight(p->left), getHeight(p->right) )+1;
    	   if(!adjusted && getHeight(p->right)-getHeight(p->left) == 2) {
    		     if(e < p->right->key)  {  // RL
    		    	   rightRotate(p->right);
    		    	   leftRotate(p);
    		     }else {   // RR
...
template<typename T,typename V>
int AVLTree<T,V>::getHeight(AVLNode<T,V> *p) {
	 if (p == 0) {
		  return 0;
	 }
     return p->height;
...
void AVLTree<T,V>::leftRotate(AVLNode<T,V>* &p) {
       ...
       p = rc;
       //update height
       p->left->height = std::max( getHeight(p->left->left), getHeight(p->left->right) )+1;
       p->height = std::max( getHeight(p->left), getHeight(p->right) )+1;
}
/**
...
void AVLTree<T,V>::rightRotate(AVLNode<T,V>* &p) {
	...
	p = lc;
	//update height
	p->right->height = std::max( getHeight(p->right->left), getHeight(p->right->right) )+1;
    p->height = std::max( getHeight(p->left),getHeight(p->right) )+1;
}
